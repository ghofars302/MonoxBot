...
module.exports = {
    ...
    run: async function (ctx, { args }) {
        ...
        const message = {
            content: args.join(' ').replace(regex, (match, id) => (ctx.guild && ctx.guild.members.has(id) && ctx.guild.members.has(id).nickname) ? `[@${ctx.guild.members.get(id).nickname}]` : ctx.users.has(id) ? `[@${ctx.users.get(id).username}]` : match) // eslint-disable-line no-confusing-arrow
        };
        ...
        if (IdOrNot.toLowerCase() === 'id') {
            ...
            try {
                ...
                message.content = messageObject.content !== '' ? messageObject.content.replace(regex, (match, id) => (ctx.guild && ctx.guild.members.has(id) && ctx.guild.members.get(id).nickname) ? `[@${ctx.guild.members.get(id).nickname}]` : ctx.users.has(id) ? `[@${ctx.users.get(id).username}]` : match) : ''; // eslint-disable-line
        ...
        if (regex.test(userOMention)) {
            user = ctx.users.get(userOMention.replace(regex, (match, id) => (ctx.guild && ctx.guild.members.has(id)) ? `[${ctx.guild.members.get(id).nickname}]` : ctx.users.has(id) ? id : ctx.author.id)) // eslint-disable-line
        ...
        if (ctx.guild) {
            const member = ctx.guild.member(user);
