module.exports = {
	description: 'Blacklist user from use the bot again.',
    category: 'adminOnly',
    args: '(user...)',
	adminOnly: true,
	run: async function (ctx, args, argsString) {
        ...
        if (!UserID.includes(member.id)) return ctx.send(':x: ``That user NOT in the list``');
        ...
        fs.readFile('config/blacklist.json', function(err, data) {
            ...
            fs.writeFile('config/blacklist.json', jsonRaw, 'utf8', function(err) {
                ...
                ctx.send(`:white_check_mark: \`\`Success remove ${member.tag} from blacklist.\`\``);
            })
        })
	}
}
